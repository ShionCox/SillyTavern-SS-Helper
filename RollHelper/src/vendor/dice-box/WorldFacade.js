...
class WorldFacade {
		...
		this.#DicePhysics.onmessage = (e) => {
			switch( e.data.action ) {
				...
				case "collision": // Handle collision events from the worker
                    ...
                    break;
				case "die-state":
					this.#handleDieStateResponse(e.data)
					break;
			}
    }
  ...
  async init() {
	 ...
	 * @param {{position?: {x:number,y:number,z:number}, rotation?: {x:number,y:number,z:number,w:number}, scaleMultiplier?: number, wake?: boolean}} transform 目标位姿、缩放倍率与是否唤醒物理模拟。
	 * @returns {WorldFacade} 当前实例，便于链式调用。
	 */
	setDieTransform(id, {position, rotation, scaleMultiplier, wake = false} = {}) {
		if(!this.#DicePhysics || id === undefined || (!position && !rotation && !scaleMultiplier)) {
			return this
		}

		this.#DicePhysics.postMessage({
			action: "setDieTransform",
			id,
			position,
			rotation,
			scaleMultiplier,
		...
		if(this.#DiceWorld?.setDieTransform){
			this.#DiceWorld.setDieTransform({id, position, rotation, scaleMultiplier})
		}

		if(wake && this.#DiceWorld?.resumeSimulation){
			this.#DiceWorld.resumeSimulation(false)
		}
	...
	/**
	 * 功能：仅更新骰子的显示层位姿，不影响物理结果。
	 * @param {number|string} id 骰子唯一标识。
	 * @param {{position?: {x:number,y:number,z:number}, rotation?: {x:number,y:number,z:number,w:number}, scaleMultiplier?: number}} transform 显示层位姿数据。
	 * @returns {WorldFacade} 当前实例，便于链式调用。
	 */
	setDieVisualTransform(id, {position, rotation, scaleMultiplier} = {}) {
		if(id === undefined || (!position && !rotation && !scaleMultiplier)) {
			return this
		}
	 ...
	 * 功能：仅移动指定骰子的位置。
	 * @param {number|string} id 骰子唯一标识。
	 * @param {{x:number,y:number,z:number}} position 目标位置。
	 * @param {{wake?: boolean}=} options 附加配置。
	 * @returns {WorldFacade} 当前实例，便于链式调用。
	 */
	moveDie(id, position, {wake = false} = {}) {
		return this.setDieTransform(id, {
			position,
			wake
		})
	}
	 ...
	 * 功能：仅旋转指定骰子。
	 * @param {number|string} id 骰子唯一标识。
	 * @param {{x:number,y:number,z:number,w:number}} rotation 目标四元数旋转。
	 * @param {{wake?: boolean}=} options 附加配置。
	 * @returns {WorldFacade} 当前实例，便于链式调用。
	 */
	rotateDie(id, rotation, {wake = false} = {}) {
		return this.setDieTransform(id, {
			rotation,
			wake
		})
	...
	/**
	 * 功能：对指定骰子施加冲量，便于做二次推动或弹射效果。
	 * @param {number|string} id 骰子唯一标识。
	 * @param {{x:number,y:number,z:number}} impulse 冲量向量。
	 * @param {{x:number,y:number,z:number}=} point 冲量作用点，省略时默认作用在骰子中心附近。
	 * @returns {WorldFacade} 当前实例，便于链式调用。
	 */
	applyImpulse(id, impulse, point) {
		if(!this.#DicePhysics || id === undefined || !impulse) {
			return this
		}
		...
		if(this.#DiceWorld?.resumeSimulation){
			this.#DiceWorld.resumeSimulation(false)
		}
	 ...
	 * 功能：读取指定骰子的当前物理状态。
	 * @param {number|string} id 骰子唯一标识。
	 * @returns {Promise<object|null>} 返回骰子状态；若不存在则返回空值。
	 */
	getDieState(id) {
		if(!this.#DicePhysics || id === undefined) {
		...
		return new Promise((resolve) => {
			this.pendingDieStateRequests[requestId] = resolve
			this.#DicePhysics.postMessage({
				action: "getDieState",
				id,
				requestId
	...
	/**
	 * 功能：读取聚焦阶段用于显示层摆正的调试快照。
	 * @param {number|string} id 骰子唯一标识。
	 * @param {{dieType?: string|null, resultValue?: number|string|null, poseEulerDeg?: {x:number,y:number,z:number}|null}=} options 额外配置。
	 * @returns {{rotation:{x:number,y:number,z:number,w:number}, poseEulerDeg:{x:number,y:number,z:number}, resultKey:string|null}|null} 返回调试快照。
	 */
	getDieFocusSnapshot(id, options = {}) {
		if(id === undefined || !this.#DiceWorld?.getDieFocusSnapshot) {
			return null
		}

		return this.#DiceWorld.getDieFocusSnapshot(id, options)
	}

	/**
	 * 功能：读取聚焦阶段用于显示层摆正的目标旋转。
	 * @param {number|string} id 骰子唯一标识。
	 * @param {{dieType?: string|null, resultValue?: number|string|null, poseEulerDeg?: {x:number,y:number,z:number}|null}=} options 额外配置。
	 * @returns {{x:number,y:number,z:number,w:number}|null} 返回目标四元数；不可计算时返回空值。
	 */
	getDieFocusRotation(id, options = {}) {
		if(id === undefined || !this.#DiceWorld?.getDieFocusRotation) {
			return null
		}

		return this.#DiceWorld.getDieFocusRotation(id, options)
	}

	/**
	 * 功能：返回当前渲染场景，供测试页挂载旋转把手。
	 * @returns {object|null} 当前场景实例。
	 */
	getScene() {
		return this.#DiceWorld?.getScene?.() ?? null
	}

	/**
	 * 功能：返回指定骰子的显示网格，供测试页挂载旋转把手。
	 * @param {number|string} id 骰子唯一标识。
	 * @returns {object|null} 骰子网格。
	 */
	getDieMesh(id) {
		return this.#DiceWorld?.getDieMesh?.(id) ?? null
	}

	/**
	 * 功能：返回指定骰子的运行时数据，供校准页做复合骰分组。
	 * @param {number|string} id 骰子唯一标识。
	 * @returns {{id:number|string, dieType:string|null, value:number|null, hasPartner:boolean, parentId:number|string|null}|null} 骰子数据。
	 */
	getDieData(id) {
		return this.#DiceWorld?.getDieData?.(id) ?? null
	}
  ...
  parse(notation, diceAvailable) {
	...
	/**
	 * 功能：处理物理线程返回的骰子状态查询结果。
	 * @param {{requestId:number, state?:object|null}} data 物理线程返回的数据。
	 * @returns {void}
	 */
	#handleDieStateResponse(data) {
		const resolver = this.pendingDieStateRequests[data.requestId]
		if(!resolver) {
			return
		}
		delete this.pendingDieStateRequests[data.requestId]
		resolver(data.state ?? null)
	}
}
