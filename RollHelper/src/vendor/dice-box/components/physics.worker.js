    ...
    case "clearDice":
      ...
      break
		...
		case "setDieTransform":
			setDieTransform(e.data)
			break
		case "applyImpulse":
			applyImpulseToDie(e.data)
			break
		case "getDieState":
			self.postMessage({
				action: "die-state",
				requestId: e.data.requestId,
				state: getDieState(e.data.id)
			})
			break
    case "connect":
...
/**
 * 功能：按标识查找正在运动或已休眠的骰子刚体。
 * @param {number|string} id 骰子唯一标识。
 * @returns {{body: object|null, source: "bodies"|"sleepingBodies"|null, index: number}} 返回匹配结果。
 */
const findBodyById = (id) => {
	const idText = `${id}`
	const activeIndex = bodies.findIndex((die) => `${die.id}` === idText)
	if(activeIndex >= 0) {
		return {
	...
	const sleepingIndex = sleepingBodies.findIndex((die) => `${die.id}` === idText)
	if(sleepingIndex >= 0) {
		return {
			body: sleepingBodies[sleepingIndex],
			source: "sleepingBodies",
			index: sleepingIndex
		}
	}
...
/**
 * 功能：唤醒休眠骰子，使其重新参与物理模拟。
 * @param {number|string} id 骰子唯一标识。
 * @returns {object|null} 返回被唤醒后的刚体。
 */
const wakeBodyById = (id) => {
	const match = findBodyById(id)
	if(!match.body) {
		return null
	}
	...
	if(match.source === "sleepingBodies") {
		sleepingBodies.splice(match.index, 1)
		bodies.push(body)
	}

	const localInertia = new Ammo.btVector3(0, 0, 0)
	body.getCollisionShape().calculateLocalInertia(body.mass, localInertia)
	body.setMassProps(body.mass, localInertia)
...
/**
 * 功能：设置指定骰子的物理位姿。
 * @param {{id:number|string, position?:{x:number,y:number,z:number}, rotation?:{x:number,y:number,z:number,w:number}, wake?:boolean}} data 骰子位姿数据。
 * @returns {void}
 */
const setDieTransform = (data) => {
	const body = data.wake ? wakeBodyById(data.id) : findBodyById(data.id).body
	if(!body) {
		return
	}
	...
	if(data.position) {
		origin.setValue(data.position.x, data.position.y, data.position.z)
		transform.setOrigin(origin)
	}

	if(data.rotation) {
		quaternion.setValue(data.rotation.x, data.rotation.y, data.rotation.z, data.rotation.w)
...
/**
 * 功能：对指定骰子施加冲量。
 * @param {{id:number|string, impulse:{x:number,y:number,z:number}, point?:{x:number,y:number,z:number}}} data 冲量数据。
 * @returns {void}
 */
const applyImpulseToDie = (data) => {
	const body = wakeBodyById(data.id)
	if(!body || !data.impulse) {
		return
	}
 ...
 * 功能：读取指定骰子的当前物理状态。
 * @param {number|string} id 骰子唯一标识。
 * @returns {object|null} 返回骰子的位姿与速度信息。
 */
const getDieState = (id) => {
	const body = findBodyById(id).body
	if(!body) {
		return null
	}
