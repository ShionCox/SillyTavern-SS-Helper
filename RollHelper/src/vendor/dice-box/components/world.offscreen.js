...
class WorldOffScreen {
	...
	resumeSimulation(newStartPoint = false){
		this.#OffscreenWorker.postMessage({action: "resumeSimulation", newStartPoint})
	}

	/**
	 * 功能：立即同步单颗骰子的显示位姿。
	 * @param {{id:number|string, position?:{x:number,y:number,z:number}, rotation?:{x:number,y:number,z:number,w:number}}} options 骰子位姿数据。
	 * @returns {void}
	 */
	setDieTransform(options){
		this.#OffscreenWorker.postMessage({action: "setDieTransform", options})
	}
