...
export function buildSettingsCardHtmlTemplateEvent(ids: SettingsCardTemplateIdsEvent): string {
  const contentHtml = `
    ...
    "为主流助手等功能自动移除隐藏内容代码块",
    "在处理消息回调以提供上下文时删除所有隐藏块与包裹，防止它们被错误引用或展示（例如摘要和记忆插件）。",
    ...
    "确保原始消息不受隐藏数据污染",
    "开启此功能后，每当系统获取最近的一条助手消息并执行相关内容附加操作后，都会把其内部包含的 RollJson 原始块一并去除。若关闭此选项，RollHelper 依赖于 ST UI 渲染逻辑在 UI 进行遮盖，其他读取直接消息的对象（如酒馆指令）将会抓取到它。",
    ...
    "清理自定义标记",
    "同时净化掉本程序产生的诸如 <!-- ROLLHELPER_SUMMARY_START --> 一类自定义标签。",
    "compatibility internal regex script tag block wrapper"
          ...
          <div class="st-roll-item st-roll-search-item" data-st-roll-search="clean history reset sanitize text chat blocks debug">
            <div class="st-roll-item-main">
              ...
              <div class="st-roll-item-desc">清空并过滤当前当前所选聊天历史记录里所有 AI 发出的由于之前版本没有做该兼容性处理而留下的 rolljson 隐藏块。（建议开启上方三个功能后在所有含有骰子的老对话里使用一遍。）</div>
            </div>
