...
export function cleanAllHistoryChatBlocks(
    chatContext: any[],
    options?: { blockInternalTags?: boolean; forceAll?: boolean }
): number {
    if (!Array.isArray(chatContext)) return 0;
    let cleanedCount = 0;
    ...
    for (let i = 0; i < chatContext.length; i++) {
        ...
        // 默认只对 AI 的消息或明确包含相关标记的消息做深度净化
        if (!options?.forceAll && msg.is_user) {
            continue;
        }

        try {
            const rawText = getMessageTextSafe(msg);
            if (!rawText) continue;

            const cleanedText = stripRollJsonBlocks(rawText);
            if (rawText !== cleanedText) {
                setMessageTextSafe(msg, cleanedText);
            ...
            console.warn(`[RollHelper] Error cleaning message historical block at index ${i}`, err);
        }
    }
