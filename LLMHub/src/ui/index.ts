...
type LLMHubRuntime = {
  ...
  getStatusSnapshot?: () => Promise<
    import("../schema/types").LLMHubStatusSnapshot
  >;
  previewRoute?: (
    args: import("../schema/types").RouteResolveArgs,
  ) => Promise<import("../schema/types").RoutePreviewSnapshot>;
  setBudgetConfig?: (consumer: string, config: BudgetConfig) => void;
  removeBudgetConfig?: (consumer: string) => void;
  listRequestLogs?: (
    opts?: LLMRequestLogQueryOptions,
...
function buildRequestLogOutboundPreview(entry: LLMRequestLogEntry): string {
  const outbound =
    entry.request?.providerRequest ??
    (entry.taskKind === "embedding"
      ? {
          texts: Array.isArray(entry.request?.embeddingTexts)
            ? entry.request.embeddingTexts
            : [],
      ...
      : entry.taskKind === "rerank"
        ? {
            ...
            docs: Array.isArray(entry.request?.rerankDocs)
              ? entry.request.rerankDocs
              : [],
...
function normalizeCustomParams(
  value: unknown,
): ResourceCustomParams | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return undefined;
  }
  const entries = Object.entries(value).filter(
    ([key]: [string, unknown]) => String(key || "").trim().length > 0,
  );
...
function normalizeResourceCapabilities(
  ...
  if (
    ...
    declaredCapabilities.includes("rerank")
  ) {
...
function serializeCustomParamValue(
  value: unknown,
  type: ResourceCustomParamValueType,
): string {
...
function buildCustomParamRows(
  params?: ResourceCustomParams,
): ResourceCustomParamRow[] {
  if (!params) return [];
...
function normalizeResourceConfigForUi(config: ResourceConfig): ResourceConfig {
  const normalizedApiType: ApiType | undefined =
    config.source === "custom" && config.type === "generation"
  ...
  const normalizedBase: ResourceConfig = {
    ...
    label:
      String(config.label || config.id || "").trim() || String(config.id || ""),
    baseUrl:
      config.source === "custom"
        ? String(config.baseUrl || "").trim() || undefined
        : undefined,
    model: String(config.model || "").trim() || undefined,
    enabled: config.enabled !== false,
    rerankPath:
      config.type === "rerank"
        ? String(config.rerankPath || "").trim() || undefined
        : undefined,
...
function normalizeGlobalMaxTokensControl(
  value: unknown,
): GlobalMaxTokensControl {
  const raw =
    value && typeof value === "object" ? (value as GlobalMaxTokensControl) : {};
  const mode =
    raw.mode === "manual" || raw.mode === "adaptive" ? raw.mode : "inherit";
  const manualValue = sanitizePositiveInteger(raw.manualValue);
  return {
    mode,
    manualValue,
    adaptive:
      raw.adaptive && typeof raw.adaptive === "object"
        ? {
            ...
            schemaCharDivisor: sanitizePositiveInteger(
              raw.adaptive.schemaCharDivisor,
            ),
...
function formatTaskDisplayLabel(
  taskKey?: string,
  taskDescription?: string,
): string {
...
function formatAttemptBadgeLabel(entry: LLMRequestLogEntry): string {
  if (entry.attemptTag === "重试" && entry.attemptOutcome === "成功")
    return "重试成功";
  if (entry.attemptTag === "重试" && entry.attemptOutcome === "失败")
    return "重试失败";
  if (entry.attemptTag === "重试" && entry.attemptOutcome === "取消")
    return "已取消";
  if (entry.attemptTag === "初次请求" && entry.attemptOutcome === "失败")
    return "初次失败";
  if (entry.attemptTag === "初次请求" && entry.attemptOutcome === "成功")
    return "初次成功";
...
function formatDiscoveredConsumerSummary(
  consumers: DiscoveredConsumer[],
): string {
  ...
  const names = consumers
    ...
    .map(
      (consumer: DiscoveredConsumer) =>
        consumer.displayName || consumer.pluginId,
    );
  const summary = names.join("、");
  return consumers.length <= 3
    ? summary
    : `${summary} 等 ${consumers.length} 个插件`;
}

function buildPluginAssignmentsEmptyStateHtml(
  consumers: DiscoveredConsumer[],
): string {
  const onlineConsumers = consumers.filter(
    (consumer: DiscoveredConsumer) => consumer.alive === true,
  );
  const memoryOsConsumer = onlineConsumers.find(
    (consumer: DiscoveredConsumer) => consumer.pluginId === "stx_memory_os",
  );
  ...
  if (onlineConsumers.length > 0) {
    const summary = escapeHtml(
      formatDiscoveredConsumerSummary(onlineConsumers),
    );
...
function generateChangelogHtml(): string {
  ...
  return changelogData
    .map(
      (log: { version: string; date?: string; changes?: string[] }) => `
...
const IDS: LLMHubSettingsIds = {
  ...
  githubText: (manifestJson as any).homePage
    ? (manifestJson as any).homePage.replace(/^https?:\/\//i, "")
    : "GitHub",
...
function bindUiEvents(): void {
  ...
  const getStContext = (): any =>
    (window as any).SillyTavern?.getContext?.() || null;

  const ensureSettings = (): LLMHubSettings => {
    const ctx = getStContext();
    if (!ctx) return {};
    if (!ctx.extensionSettings) ctx.extensionSettings = {};
    if (!ctx.extensionSettings.stx_llmhub)
      ctx.extensionSettings.stx_llmhub = {};
  ...
  if (searchInput) {
    searchInput.addEventListener("input", (evt: Event) => {
      ...
      searchableItems.forEach((el: Element) => {
        const keywords = (
          el.getAttribute("data-stx-ui-search") || ""
        ).toLowerCase();
  ...
  const globalMaxTokensModeEl = document.getElementById(
    IDS.globalMaxTokensModeId,
  ) as HTMLSelectElement | null;
  const globalMaxTokensManualEl = document.getElementById(
    IDS.globalMaxTokensManualId,
  ) as HTMLInputElement | null;

  const syncGlobalMaxTokensUi = (): void => {
    const control = normalizeGlobalMaxTokensControl(
      ensureSettings().maxTokensControl,
    );
    if (globalMaxTokensModeEl)
      globalMaxTokensModeEl.value = control.mode || "inherit";
    if (globalMaxTokensManualEl) {
      globalMaxTokensManualEl.value = control.manualValue
        ? String(control.manualValue)
        : "";
      globalMaxTokensManualEl.disabled = control.mode !== "manual";
      globalMaxTokensManualEl.placeholder =
        control.mode === "manual" ? "例如 1600" : "仅手动模式可编辑";
    }
    syncSharedSelects(cardRoot || document.body);
  };
  ...
  syncGlobalMaxTokensUi();
  globalMaxTokensModeEl?.addEventListener(
    "change",
    persistGlobalMaxTokensControl,
  );
  globalMaxTokensManualEl?.addEventListener(
    "change",
    persistGlobalMaxTokensControl,
  );
  globalMaxTokensManualEl?.addEventListener(
  ...
  const resourceEditorEl = document.getElementById(
    IDS.resourceEditorId,
  ) as HTMLElement | null;
  const resourceTypeSelectEl = document.getElementById(
    IDS.resourceTypeSelectId,
  ) as HTMLSelectElement | null;
  const resourceSourceSelectEl = document.getElementById(
    IDS.resourceSourceSelectId,
  ) as HTMLSelectElement | null;
  const resourceApiTypeSelectEl = document.getElementById(
    IDS.resourceApiTypeSelectId,
  ) as HTMLSelectElement | null;
  const resourceBaseUrlEl = document.getElementById(
    IDS.resourceBaseUrlId,
  ) as HTMLInputElement | null;
  ...
  const resourceRerankPathEl = document.getElementById(
    IDS.resourceRerankPathId,
  ) as HTMLInputElement | null;
  const resourceCustomParamsEl = document.getElementById(
    IDS.resourceCustomParamsId,
  ) as HTMLTextAreaElement | null;
  const resourceCustomParamsListEl = document.getElementById(
    IDS.resourceCustomParamsListId,
  ) as HTMLElement | null;
  const resourceCustomParamsAddBtn = document.getElementById(
    IDS.resourceCustomParamsAddBtnId,
  ) as HTMLButtonElement | null;
  const resourceCustomParamsSyncBtn = document.getElementById(
    IDS.resourceCustomParamsSyncBtnId,
  ) as HTMLButtonElement | null;
  const rerankTestPanelEl = document.getElementById(
    IDS.rerankTestPanelId,
  ) as HTMLElement | null;
  const rerankTestQueryEl = document.getElementById(
    IDS.rerankTestQueryId,
  ) as HTMLInputElement | null;
  const rerankTestDocsEl = document.getElementById(
    IDS.rerankTestDocsId,
  ) as HTMLTextAreaElement | null;
  const rerankTestTopKEl = document.getElementById(
    IDS.rerankTestTopKId,
  ) as HTMLInputElement | null;
  const resourceIdInputEl = document.getElementById(
    IDS.resourceIdInputId,
  ) as HTMLInputElement | null;
  const resourceLabelInputEl = document.getElementById(
    IDS.resourceLabelInputId,
  ) as HTMLInputElement | null;
  const resourceEnabledEl = document.getElementById(
    IDS.resourceEnabledId,
  ) as HTMLInputElement | null;
  const resourceListEl = document.getElementById(
    IDS.resourceListId,
  ) as HTMLElement | null;
  const resourceNewBtn = document.getElementById(
    IDS.resourceNewBtnId,
  ) as HTMLButtonElement | null;
  const resourceSaveBtn = document.getElementById(
    IDS.resourceSaveBtnId,
  ) as HTMLButtonElement | null;
  const resourceDeleteBtn = document.getElementById(
    IDS.resourceDeleteBtnId,
  ) as HTMLButtonElement | null;
  const testConnectionBtn = document.getElementById(
    IDS.testConnectionBtnId,
  ) as HTMLButtonElement | null;
  const testRerankBtn = document.getElementById(
    IDS.testRerankBtnId,
  ) as HTMLButtonElement | null;
  const fetchModelsBtn = document.getElementById(
    IDS.fetchModelsBtnId,
  ) as HTMLButtonElement | null;
  const testResultEl = document.getElementById(
    IDS.testResultId,
  ) as HTMLElement | null;
  const tavernInfoStatusEl = document.getElementById(
    IDS.tavernInfoStatusId,
  ) as HTMLElement | null;
  const tavernInfoListEl = document.getElementById(
    IDS.tavernInfoListId,
  ) as HTMLElement | null;
  const modelListSelectEl = document.getElementById(
    IDS.modelListSelectId,
  ) as HTMLSelectElement | null;
  const modelListStatusEl = document.getElementById(
    IDS.modelListStatusId,
  ) as HTMLElement | null;

  const globalAssignGenModelEl = document.getElementById(
    IDS.globalAssignGenModelId,
  ) as HTMLInputElement | null;
  const globalAssignEmbModelEl = document.getElementById(
    IDS.globalAssignEmbModelId,
  ) as HTMLInputElement | null;
  const globalAssignRerankModelEl = document.getElementById(
    IDS.globalAssignRerankModelId,
  ) as HTMLInputElement | null;

  [
    globalAssignGenModelEl,
    globalAssignEmbModelEl,
    globalAssignRerankModelEl,
  ].forEach((el: HTMLInputElement | null) => {
    if (!el) return;
    el.disabled = true;
    el.value = "";
  ...
  const resourceBaseUrlField = resourceBaseUrlEl?.closest(
    ".stx-ui-field",
  ) as HTMLElement | null;
  const resourceApiTypeField = resourceApiTypeSelectEl?.closest(
    ".stx-ui-field",
  ) as HTMLElement | null;
  const resourceRerankPathField = resourceRerankPathEl?.closest(
    ".stx-ui-field",
  ) as HTMLElement | null;
  const resourceCapRerankEl = document.getElementById(
    IDS.resourceCapRerankId,
  ) as HTMLInputElement | null;
  ...
  const buildRerankResultDetailHtml = (
    results: Array<{ index: number; score: number; doc: string }>,
  ): string => {
    if (!Array.isArray(results) || results.length === 0) return "";
    const items = results
      .map(
        (item: { index: number; score: number; doc: string }, rank: number) => `
  ...
  const renderCustomParamRows = (params?: ResourceCustomParams): void => {
    ...
    if (rows.length === 0) {
      resourceCustomParamsListEl.innerHTML =
        '<div class="stx-ui-param-empty">暂无参数项。可点击“添加参数”，也可直接编辑下方 JSON。</div>';
      return;
    }
    resourceCustomParamsListEl.innerHTML = rows
      .map(
        (row: ResourceCustomParamRow, index: number) => `
            <div class="stx-ui-param-row" data-param-row="${index}">
              ...
              />
                            ${buildSharedSelectField({
                              ...
                              selectClassName: "stx-ui-input stx-ui-input-full",
                              triggerClassName:
                                "stx-ui-input-full stx-shared-select-trigger-compact",
  ...
  const collectCustomParamsFromRows = (): ResourceCustomParams | undefined => {
    ...
    const rowEls = Array.from(
      resourceCustomParamsListEl.querySelectorAll<HTMLElement>(
        "[data-param-row]",
      ),
    );
    ...
    for (const rowEl of rowEls) {
      const key = String(
        rowEl.querySelector<HTMLInputElement>("[data-param-key]")?.value || "",
      ).trim();
      const type = (rowEl.querySelector<HTMLSelectElement>("[data-param-type]")
        ?.value || "string") as ResourceCustomParamValueType;
      const value = String(
        rowEl.querySelector<HTMLInputElement>("[data-param-value]")?.value ||
          "",
      );
      if (!key) continue;
  ...
  const syncCustomParamsTextareaFromRows = (showError = true): boolean => {
    ...
    try {
      ...
      resourceCustomParamsEl.value = params
      ...
      if (showError) {
        alert(
          `参数项校验失败：${error instanceof Error ? error.message : String(error)}`,
        );
      }
      return false;
    }
  ...
  const refreshRerankTestButtonState = (): void => {
    ...
    const source = (resourceSourceSelectEl?.value ||
      "custom") as ResourceSource;
    const type = (resourceTypeSelectEl?.value || "generation") as ResourceType;
    const rerankEnabled =
      type === "rerank" ||
      (type === "generation" &&
        source === "custom" &&
        resourceCapRerankEl?.checked === true);
  ...
  const getRerankTestPayload = (): {
    ...
    return {
      ...
      topK:
        topK != null
          ? Math.min(Math.max(1, Math.floor(topK)), docs.length)
          : undefined,
    };
  };
  ...
  const getSavedResourceOptions = (
    ...
    return getSavedResources()
      ...
      .filter((option: ResourceOption) =>
        requiredCapabilities.every((cap: LLMCapability) =>
  ...
  const syncResourceEditorState = (): void => {
    ...
    if (resourceRerankPathField)
      resourceRerankPathField.style.display = type === "rerank" ? "" : "none";
    if (type !== "rerank" && resourceRerankPathEl)
      resourceRerankPathEl.value = "";
  ...
  const loadResourceIntoEditor = (
    ...
    if (resourceLabelInputEl)
      resourceLabelInputEl.value = resource?.label || "";
    if (resourceTypeSelectEl)
      resourceTypeSelectEl.value = resource?.type || "generation";
    if (resourceSourceSelectEl)
      resourceSourceSelectEl.value = resource?.source || "custom";
    if (resourceApiTypeSelectEl)
      resourceApiTypeSelectEl.value = getApiTypeLabel(resource?.apiType);
    if (resourceBaseUrlEl)
      resourceBaseUrlEl.value =
        resource?.source === "custom" ? resource.baseUrl || "" : "";
    if (resourceDefaultModelEl)
      resourceDefaultModelEl.value = resource?.model || "";
    if (resourceRerankPathEl)
      resourceRerankPathEl.value = resource?.rerankPath || "";
    if (resourceCustomParamsEl) {
      resourceCustomParamsEl.value = resource?.customParams
        ? JSON.stringify(resource.customParams, null, 2)
        : "";
    }
    renderCustomParamRows(resource?.customParams);
    ensureRerankTestDefaults();
    if (resourceEnabledEl)
      resourceEnabledEl.checked = resource ? resource.enabled !== false : true;
    if (resourceCapRerankEl) {
      resourceCapRerankEl.checked = Boolean(
        resource?.capabilities?.includes("rerank") &&
  ...
  const renderResourceList = (): void => {
    ...
    if (resources.length === 0) {
      resourceListEl.innerHTML =
        '<div class="stx-ui-list-empty">暂无已保存资源，请先新建一条资源配置。</div>';
      return;
    }
                                ...
                                >
                                    <div class="stx-ui-resource-head">
                                        ...
                                        <div class="stx-ui-list-content">
                                            ...
                                            <div style="display: flex; gap: 8px; align-items: center; margin-top: 4px;flex-direction: row;">
  ...
  const readEditingResource = (): ResourceConfig | null => {
    ...
    if (customParamsText) {
      try {
        ...
        alert(
          `自定义参数 JSON 解析失败：${error instanceof Error ? error.message : String(error)}`,
        );
        return null;
      }
    }
    ...
    return normalizeResourceConfigForUi({
      ...
      source,
      apiType:
        source === "custom" && type === "generation" ? apiType : undefined,
      ...
      capabilities:
        canToggleRerank && resourceCapRerankEl?.checked
          ? ["rerank"]
          : undefined,
      customParams,
    });
  ...
  const collectResourceReferences = (resourceId: string): string[] => {
    ...
    if (settings.globalAssignments?.generation?.resourceId === resourceId)
      references.push("全局分配 / 生成");
    if (settings.globalAssignments?.embedding?.resourceId === resourceId)
      references.push("全局分配 / 向量化");
    if (settings.globalAssignments?.rerank?.resourceId === resourceId)
      references.push("全局分配 / 重排序");
    (settings.pluginAssignments || []).forEach((item: PluginAssignment) => {
      if (item.generation?.resourceId === resourceId)
        references.push(`插件分配 / ${item.pluginId} / 生成`);
      if (item.embedding?.resourceId === resourceId)
        references.push(`插件分配 / ${item.pluginId} / 向量化`);
      if (item.rerank?.resourceId === resourceId)
        references.push(`插件分配 / ${item.pluginId} / 重排序`);
    });
    (settings.taskAssignments || []).forEach((item: TaskAssignment) => {
      if (item.resourceId === resourceId)
        references.push(`任务分配 / ${item.pluginId} / ${item.taskKey}`);
    });
    return references;
  };

  const syncResourceListItemState = (
    resourceId: string,
    enabled: boolean,
  ): void => {
    const rowEl =
      resourceListEl?.querySelector<HTMLElement>(
    ...
    const toggleEl = rowEl.querySelector<HTMLInputElement>(
      "[data-resource-toggle]",
    );
    if (toggleEl) {
      toggleEl.checked = enabled;
      const resource =
        getSavedResources().find(
          (item: ResourceConfig) => item.id === resourceId,
        ) || null;
      const resourceLabel = resource?.label || resourceId;
      toggleEl.dataset.tip = `${enabled ? "停用" : "启用"}资源 ${resourceLabel}`;
      toggleEl.setAttribute(
        "aria-label",
        `${enabled ? "停用" : "启用"}资源 ${resourceLabel}`,
      );
    }
  };

  const updateResourceEnabled = async (
    resourceId: string,
    enabled: boolean,
  ): Promise<void> => {
    const settings = ensureSettings();
    const resources = getSavedResources();
    const target =
      resources.find((item: ResourceConfig) => item.id === resourceId) || null;
    if (!target) return;
    ...
    showTestResult({
      ok: true,
      message: `${enabled ? "已启用" : "已停用"}资源 ${target.label || target.id}`,
    });
  };
  ...
  const deleteResource = async (resourceId: string): Promise<void> => {
    ...
    const settings = ensureSettings();
    settings.resources = getSavedResources().filter(
      (item: ResourceConfig) => item.id !== resourceId,
    );

    const nextGlobalAssignments: GlobalAssignments = {
      ...(settings.globalAssignments || {}),
    };
    if (nextGlobalAssignments.generation?.resourceId === resourceId)
      delete nextGlobalAssignments.generation;
    if (nextGlobalAssignments.embedding?.resourceId === resourceId)
      delete nextGlobalAssignments.embedding;
    if (nextGlobalAssignments.rerank?.resourceId === resourceId)
      delete nextGlobalAssignments.rerank;
    ...
    settings.pluginAssignments = (settings.pluginAssignments || [])
      .map((item: PluginAssignment) => {
        ...
        if (item.generation && item.generation.resourceId !== resourceId)
          next.generation = item.generation;
        if (item.embedding && item.embedding.resourceId !== resourceId)
          next.embedding = item.embedding;
        if (item.rerank && item.rerank.resourceId !== resourceId)
          next.rerank = item.rerank;
        return next;
      })
      .filter((item: PluginAssignment) =>
        Boolean(item.generation || item.embedding || item.rerank),
      );

    settings.taskAssignments = (settings.taskAssignments || []).filter(
      (item: TaskAssignment) => item.resourceId !== resourceId,
    );
    ...
    await runtime?.applySettingsFromContext?.();
    editingResourceId =
      editingResourceId === resourceId ? "" : editingResourceId;
  ...
  resourceCustomParamsListEl?.addEventListener("click", (event: Event) => {
    const removeBtn = (event.target as HTMLElement).closest<HTMLButtonElement>(
      "[data-param-remove]",
    );
    if (!removeBtn) return;
  ...
  testConnectionBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      const runtimeProvider =
        (runtime?.router?.getProvider?.(resource.id) as
          | ProviderWithTest
          | null
          | undefined) || null;
      ...
      if (resource.type === "embedding" && runtimeProvider?.embed) {
        ...
        const first = Array.isArray(result?.embeddings)
          ? result.embeddings[0]
          : null;
        ...
        if (dim > 0) {
          ...
          showTestResult({
            ok: false,
            message: "连接失败：Embedding 返回为空或格式异常",
          });
        }
        return;
      }
  ...
  testRerankBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      const summary = (result.results || [])
        .slice(0, 2)
        .map(
          (item: { index: number; score: number; doc: string }) =>
            `#${item.index}=${item.score.toFixed(3)}：${item.doc.slice(0, 24)}`,
        )
        .join("，");
      const modeLabel =
        resource.type === "generation" ? "LLM 重排模式" : "专用重排接口";
      ...
      if (Array.isArray(result.results) && result.results.length > 0) {
        ...
        showTestResult({
          ok: false,
          message: `${modeLabel}测试失败：返回为空或格式异常`,
        });
      }
    } catch (error: unknown) {
      showTestResult({
        ok: false,
        message: `重排测试异常：${error instanceof Error ? error.message : String(error)}`,
      });
    } finally {
  ...
  fetchModelsBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!runtimeProvider?.listModels) {
        if (modelListStatusEl)
          modelListStatusEl.textContent = "当前资源不支持获取模型列表";
        return;
      }
      const result = await runtimeProvider.listModels();
      if (!result.ok) {
        if (modelListStatusEl)
          modelListStatusEl.textContent = `获取失败：${result.message}`;
        return;
      }
      ...
      if (modelListStatusEl)
        modelListStatusEl.textContent = `已获取 ${result.models.length} 个模型`;
    } catch (error: unknown) {
      if (modelListStatusEl)
        modelListStatusEl.textContent = `获取异常：${error instanceof Error ? error.message : String(error)}`;
    } finally {
  ...
  const populateResourceSelect = (
    ...
    if (
      Array.from(selectEl.options).some(
        (o: HTMLOptionElement) => o.value === prev,
      )
    ) {
      selectEl.value = prev;
    }
  ...
  const refreshAllResourceSelects = (): void => {
    populateResourceSelect(
      document.getElementById(
        IDS.globalAssignGenResourceId,
      ) as HTMLSelectElement | null,
      getSavedResourceOptions(["chat"]),
      true,
    );
    populateResourceSelect(
      document.getElementById(
        IDS.globalAssignEmbResourceId,
      ) as HTMLSelectElement | null,
      getSavedResourceOptions(["embeddings"]),
      true,
    );
    populateResourceSelect(
      document.getElementById(
        IDS.globalAssignRerankResourceId,
      ) as HTMLSelectElement | null,
      getSavedResourceOptions(["rerank"]),
      true,
    );
    refreshSharedSelectOptions(cardRoot || document.body);
  };
  ...
  const restoreGlobalAssignmentsToUI = (): void => {
    ...
    const genEl = document.getElementById(
      IDS.globalAssignGenResourceId,
    ) as HTMLSelectElement | null;
    const embEl = document.getElementById(
      IDS.globalAssignEmbResourceId,
    ) as HTMLSelectElement | null;
    const rerankEl = document.getElementById(
      IDS.globalAssignRerankResourceId,
    ) as HTMLSelectElement | null;
  ...
  document
    .getElementById(IDS.globalAssignSaveBtnId)
  ...
  const renderPluginAssignments = (): void => {
    ...
    const registrations =
      runtime?.registry?.listConsumerRegistrations?.() || [];
    if (registrations.length === 0) {
      listEl.innerHTML =
        '<div class="stx-ui-list-empty">正在检查已在线但尚未注册任务的插件…</div>';
      void discoverConsumers({
        ...
        .then((consumers: DiscoveredConsumer[]): void => {
          if (
            renderSeq !== LLMHUB_CONSUMER_DISCOVERY_SEQ ||
            !listEl.isConnected
          )
            return;
        ...
        .catch((): void => {
          if (
            ...
            return;
          listEl.innerHTML =
            '<div class="stx-ui-list-empty">暂无已注册插件</div>';
        });
      return;
    }
    ...
    listEl.innerHTML = registrations
      .map((snap: ConsumerSnapshot) => {
        ...
        const lastSeen = snap.session?.seenAt
          ? formatTimestamp(snap.session.seenAt)
          : "-";
        const declaredKinds = new Set<CapabilityKind>();
        (snap.tasks || []).forEach((task: TaskDescriptor) =>
          declaredKinds.add(task.taskKind),
        );
        const relevantKinds = kinds.filter((kind: CapabilityKind) =>
          declaredKinds.has(kind),
        );
        if (relevantKinds.length === 0) relevantKinds.push("generation");

        const existing = existingAssignments.find(
          (item: PluginAssignment) => item.pluginId === pluginId,
        );
        const kindRows = relevantKinds
          .map((kind: CapabilityKind) => {
            const requiredCaps = new Set<LLMCapability>();
            (snap.tasks || [])
              .filter((task: TaskDescriptor) => task.taskKind === kind)
              .forEach((task: TaskDescriptor) =>
                (task.requiredCapabilities || []).forEach(
                  (cap: LLMCapability) => requiredCaps.add(cap),
                ),
              );
            ...
            const selectId =
              `stx-llmhub-plugin-assign-${pluginId}-${kind}`.replace(
                /[^a-zA-Z0-9_-]/g,
                "-",
              );
            const selectHtml = buildResourceSharedSelectHtml(
              selectId,
              selected,
              getSavedResourceOptions(Array.from(requiredCaps)),
              {
  ...
  document
    .getElementById(IDS.pluginAssignListId)
    ?.addEventListener("click", (evt: Event) => {
      const target = evt.target as HTMLElement;
      const saveBtn = target.closest<HTMLButtonElement>(
        "button[data-plugin-assign-save]",
      );
      const deleteBtn = target.closest<HTMLButtonElement>(
        "button[data-plugin-assign-delete]",
      );
      const pluginId = String(
        saveBtn?.dataset.pluginAssignSave ||
          deleteBtn?.dataset.pluginAssignDelete ||
          "",
      ).trim();
      if (!pluginId) return;

      const settings = ensureSettings();
      let assignments = (settings.pluginAssignments || []).filter(
        (item: PluginAssignment) => item.pluginId !== pluginId,
      );
      ...
      if (saveBtn) {
        ...
        const selects = row.querySelectorAll<HTMLSelectElement>(
          "select[data-plugin-assign-resource]",
        );
        ...
        if (next.generation || next.embedding || next.rerank)
  ...
  document
    .getElementById(IDS.pluginAssignRefreshBtnId)
    ?.addEventListener("click", renderPluginAssignments);
   ...
   * @returns 适合展示的插件名称
   */
  const normalizeTaskAssignmentDisplayName = (
    pluginId: string,
    displayName: string,
  ): string => {
    const normalizedDisplayName = String(displayName || "").trim();
    if (
      (pluginId === "stx_memory_os" || pluginId === "memoryos") &&
      /structured memory plugin/i.test(normalizedDisplayName)
    ) {
      return "记忆系统";
    }
  ...
  const renderTaskAssignments = (): void => {
    ...
    listEl.innerHTML = allTasks
      .map(({ pluginId, displayName, task, isOnline }) => {
        const existing = existingAssignments.find(
          (item: TaskAssignment) =>
            item.pluginId === pluginId && item.taskKey === task.taskKey,
        );
        const isStale = existing?.isStale === true;
        const registeredMaxTokens =
          Number(task.maxTokens || 0) > 0 ? Number(task.maxTokens) : null;
        ...
        const selectId =
          `stx-llmhub-task-assign-${pluginId}-${task.taskKey}`.replace(
            /[^a-zA-Z0-9_-]/g,
            "-",
          );
        const selectHtml = buildResourceSharedSelectHtml(
          selectId,
          existing?.resourceId || "",
          getSavedResourceOptions(task.requiredCapabilities || []),
          {
            "data-task-assign-resource": key,
          },
        );
        const normalizedDisplayName = normalizeTaskAssignmentDisplayName(
          pluginId,
          displayName,
        );
        const capabilityText = task.requiredCapabilities?.length
          ? task.requiredCapabilities.join(" · ")
          : "";
        const statusText = isOnline ? "在线" : "离线";
        const defaultMaxTokensText = resolvedRegisteredMaxTokens
          ? `默认 ${resolvedRegisteredMaxTokens}`
          : "";
  ...
  document
    .getElementById(IDS.taskAssignListId)
    ?.addEventListener("click", (evt: Event) => {
      const target = evt.target as HTMLElement;
      const saveBtn = target.closest<HTMLButtonElement>(
        "button[data-task-assign-save]",
      );
      const deleteBtn = target.closest<HTMLButtonElement>(
        "button[data-task-assign-delete]",
      );
      const key = String(
        saveBtn?.dataset.taskAssignSave ||
          deleteBtn?.dataset.taskAssignDelete ||
          "",
      ).trim();
      if (!key) return;
      ...
      const settings = ensureSettings();
      let assignments = (settings.taskAssignments || []).filter(
        (item: TaskAssignment) =>
          !(item.pluginId === pluginId && item.taskKey === taskKey),
      );
      const registrations =
        runtime?.registry?.listConsumerRegistrations?.() || [];
      ...
      for (const snap of registrations) {
        ...
        registeredTask = snap.tasks?.find(
          (task: TaskDescriptor) => task.taskKey === taskKey,
        );
        if (registeredTask) break;
      }
      const registeredMaxTokens =
        Number(registeredTask?.maxTokens || 0) > 0
          ? Number(registeredTask?.maxTokens)
          : undefined;

      if (saveBtn) {
        const resourceEl = document.querySelector<HTMLSelectElement>(
          `select[data-task-assign-resource="${key}"]`,
        );
        const resourceId = resourceEl?.value.trim() || "";
        const maxTokensEl = document.querySelector<HTMLInputElement>(
          `input[data-task-assign-max-tokens="${key}"]`,
        );
        const rawMaxTokens = sanitizePositiveInteger(maxTokensEl?.value);
        const maxTokens =
          rawMaxTokens &&
          registeredMaxTokens &&
          rawMaxTokens === registeredMaxTokens
            ? undefined
            : rawMaxTokens;
        const taskKind = (saveBtn.dataset.taskKind ||
          "generation") as CapabilityKind;

        if (resourceId) {
          if (registeredTask?.requiredCapabilities?.length) {
            const resource = getSavedResources().find(
              (item: ResourceConfig) => item.id === resourceId,
            );
            const resourceCaps = resource
              ? normalizeResourceCapabilities(resource)
              : [];
            const missing = registeredTask.requiredCapabilities.filter(
              (cap: LLMCapability) => !resourceCaps.includes(cap),
            );
            if (missing.length > 0) {
              alert(
                `资源 "${resourceId}" 缺少所需能力: ${missing.join(", ")}。无法保存。`,
              );
              return;
            }
          }
  ...
  document
    .getElementById(IDS.taskAssignRefreshBtnId)
    ?.addEventListener("click", renderTaskAssignments);
  ...
  LLMHUB_REGISTRY_SUBSCRIPTION_DISPOSE =
    runtime?.registry?.subscribe?.((): void => {
      renderConsumerDrivenViews();
    }) || null;

  const budgetConsumerEl = document.getElementById(
    IDS.budgetConsumerId,
  ) as HTMLInputElement | null;
  const budgetMaxRpmEl = document.getElementById(
    IDS.budgetMaxRpmId,
  ) as HTMLInputElement | null;
  const budgetMaxTokensEl = document.getElementById(
    IDS.budgetMaxTokensId,
  ) as HTMLInputElement | null;
  const budgetMaxLatencyEl = document.getElementById(
    IDS.budgetMaxLatencyId,
  ) as HTMLInputElement | null;
  const budgetMaxCostEl = document.getElementById(
    IDS.budgetMaxCostId,
  ) as HTMLInputElement | null;
  ...
  const renderBudgets = (): void => {
    ...
    if (entries.length === 0) {
      budgetListEl.innerHTML =
        '<div class="stx-ui-list-empty">暂无预算规则</div>';
      return;
    }
    budgetListEl.innerHTML = entries
      .map(
        ([consumer, config]: [string, BudgetConfig]) => `
  ...
  const renderQueueSnapshot = (): void => {
    ...
    listEl.innerHTML =
      items.length > 0
        ? items.join("")
        : '<div class="stx-ui-list-empty">队列为空</div>';
  };

  const requestLogModal = document.getElementById(
    IDS.requestLogModalId,
  ) as HTMLDialogElement | null;
  const requestLogSearchInput = document.getElementById(
    IDS.requestLogSearchId,
  ) as HTMLInputElement | null;
  const requestLogStateInput = document.getElementById(
    IDS.requestLogStateFilterId,
  ) as HTMLSelectElement | null;
  const requestLogSourceInput = document.getElementById(
    IDS.requestLogSourceFilterId,
  ) as HTMLSelectElement | null;
  ...
  const requestLogClearBtn = document.getElementById(
    IDS.requestLogClearBtnId,
  ) as HTMLButtonElement | null;
  const requestLogOpenBtn = document.getElementById(
    IDS.requestLogOpenBtnId,
  ) as HTMLButtonElement | null;
  const requestLogCloseBtn = document.getElementById(
    IDS.requestLogModalCloseId,
  ) as HTMLButtonElement | null;
  const requestLogRefreshBtn = document.getElementById(
    IDS.requestLogRefreshBtnId,
  ) as HTMLButtonElement | null;
  ...
  const syncRequestLogSourceOptions = (): void => {
    ...
    const currentValue =
      String(requestLogSourceInput.value || "all").trim() || "all";
    const sourcePluginIds = Array.from(
      new Set(
        requestLogAllEntries
          .map((entry: LLMRequestLogEntry): string =>
            String(entry.sourcePluginId || "").trim(),
          )
          .filter(Boolean),
      ),
    ...
    requestLogSourceInput.innerHTML = [
      ...
      ...sourcePluginIds.map(
        (sourcePluginId: string): string =>
          `<option value="${escapeHtml(sourcePluginId)}">${escapeHtml(sourcePluginId)}</option>`,
      ),
    ].join("");

    requestLogSourceInput.value = sourcePluginIds.includes(currentValue)
      ? currentValue
      : "all";
  };

  const buildRequestLogPreviewSectionHtml = (
    title: string,
    value: string,
    copyKind: "outbound" | "inbound",
  ): string => `
  ...
  const buildRequestLogSummarySectionHtml = (
    entry: LLMRequestLogEntry,
  ): string => {
    const model = String(entry.response?.meta?.model || "-").trim() || "-";
    const resourceId =
      String(entry.response?.meta?.resourceId || "-").trim() || "-";
    const strictCompatible = entry.request?.strictSchemaCompatible;
    const originalStrictCompatible =
      entry.request?.originalStrictSchemaCompatible;
    ...
    const schemaCompatMode = String(
      entry.request?.schemaCompatMode || "",
    ).trim();
    const originalStrictIncompatibilityPath = String(
      entry.request?.originalStrictIncompatibilityPath || "",
    ).trim();
    const originalStrictIncompatibilityReason = String(
      entry.request?.originalStrictIncompatibilityReason || "",
    ).trim();
    const providerStrictIncompatibilityPath = String(
      entry.request?.providerStrictIncompatibilityPath || "",
    ).trim();
    const providerStrictIncompatibilityReason = String(
      entry.request?.providerStrictIncompatibilityReason || "",
    ).trim();
    const responseFormat = String(
      entry.request?.responseFormatResolved || "",
    ).trim();
    const responseFormatBeforeCompat = String(
      entry.request?.responseFormatBeforeCompat || "",
    ).trim();
    const responseFormatAfterCompat = String(
      entry.request?.responseFormatAfterCompat || "",
    ).trim();
    ...
    if (schemaAutofillApplied !== undefined) {
      lines.push(
        `Schema 自动填充：${schemaAutofillApplied ? "已启用" : "未启用"}`,
      );
    }
    if (schemaCompatMode) {
    ...
    if (
      originalStrictIncompatibilityPath ||
      originalStrictIncompatibilityReason
    ) {
      lines.push(
        `原始 Schema 不兼容点：${originalStrictIncompatibilityPath || "$"} | ${originalStrictIncompatibilityReason || "-"}`,
      );
    }
    if (
      providerStrictIncompatibilityPath ||
      providerStrictIncompatibilityReason
    ) {
      lines.push(
        `发送 Schema 不兼容点：${providerStrictIncompatibilityPath || "$"} | ${providerStrictIncompatibilityReason || "-"}`,
      );
    }
    if (responseFormat) {
  ...
  const renderRequestLogDetail = (entry: LLMRequestLogEntry | null): void => {
    ...
    if (!entry) {
      requestLogDetailEl.innerHTML =
        '<div class="stx-ui-list-empty">暂无可显示的日志详情</div>';
      return;
    }

    requestLogDetailEl.innerHTML = [
      buildRequestLogSummarySectionHtml(entry),
      buildRequestLogPreviewSectionHtml(
        "原始发送内容",
        buildRequestLogOutboundPreview(entry),
        "outbound",
      ),
      buildRequestLogPreviewSectionHtml(
        "原始返回内容",
        buildRequestLogInboundPreview(entry),
        "inbound",
      ),
    ].join("");
  };
  ...
  const renderRequestLogList = (): void => {
    ...
    if (!requestLogFilteredEntries.length) {
      requestLogListEl.innerHTML =
        '<div class="stx-ui-list-empty">当前筛选条件下没有日志</div>';
      renderRequestLogDetail(null);
      return;
    }

    requestLogListEl.innerHTML = requestLogFilteredEntries
      .map((entry) => {
        ...
        const formatLabel =
          entry.request?.responseFormatResolved === "json_schema"
    ...
    const selected =
      requestLogFilteredEntries.find(
        (entry) => entry.logId === requestLogSelectedId,
      ) || requestLogFilteredEntries[0];
    requestLogSelectedId = selected.logId;
    renderRequestLogDetail(selected);
  };

  const applyRequestLogFilters = (): void => {
    const search = String(requestLogSearchInput?.value || "")
      .trim()
      .toLowerCase();
    const state = (String(requestLogStateInput?.value || "all").trim() ||
      "all") as RequestState | "all";
    ...
    requestLogFilteredEntries = requestLogAllEntries.filter((entry) => {
      ...
      if (sourcePluginId !== "all" && entry.sourcePluginId !== sourcePluginId)
        return false;
  ...
  const refreshRequestLogModal = async (): Promise<void> => {
    const entries = await runtime?.listRequestLogs?.({
      order: "desc",
      limit: 500,
    });
    ...
    logger.info("[RequestLogUI][Refresh]", {
      ...
      sample: requestLogAllEntries
        .slice(0, 5)
        .map((entry: LLMRequestLogEntry) => ({
  ...
  requestLogModal?.addEventListener("click", (event: Event) => {
    ...
    if (
      event.target === requestLogModal ||
      target?.dataset.logModalRole === "backdrop"
    ) {
      closeRequestLogModal();
    }
  });
  ...
  requestLogDetailEl?.addEventListener("click", async (event: Event) => {
    ...
    const selectedEntry =
      requestLogFilteredEntries.find(
        (entry: LLMRequestLogEntry) => entry.logId === requestLogSelectedId,
      ) ||
      requestLogAllEntries.find(
        (entry: LLMRequestLogEntry) => entry.logId === requestLogSelectedId,
      ) ||
      null;
  ...
  const renderSilentPermissions = (): void => {
    ...
    const permissions =
      runtime?.displayController?.exportSilentPermissions?.() || [];
    if (permissions.length === 0) {
      listEl.innerHTML =
        '<div class="stx-ui-list-empty">暂无静默权限授权</div>';
      return;
    }

    listEl.innerHTML = permissions
      .map(
        (permission) => `
          <div class="stx-ui-list-item">
            <div>
  ...
  document
    .getElementById(IDS.silentPermissionsListId)
    ?.addEventListener("click", (evt: Event) => {
      const button = (evt.target as HTMLElement).closest<HTMLButtonElement>(
        "button[data-silent-revoke]",
      );
      if (!button) return;
  ...
  document
    .getElementById(IDS.queueRefreshBtnId)
    ?.addEventListener("click", () => {
      renderQueueSnapshot();
      renderSilentPermissions();
    });
