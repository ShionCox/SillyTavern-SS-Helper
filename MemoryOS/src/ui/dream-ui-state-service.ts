...
export class DreamUiStateService {
    ...
    async getSnapshot(): Promise<DreamUiStateSnapshot> {
        ...
        const activeMeta = activeDreamId
            ? metas.find((m) => m.dreamId === activeDreamId) ?? null
            : metas.find((m) => m.status === 'running' || m.status === 'queued') ?? null;

        // 聚合待审批
        const pendingMetas = metas.filter((m) => m.status === 'generated');
        ...
        // 最近完成
        const completedMeta = metas.find((m) => m.status === 'approved' || (m.status === 'generated' && !pendingDreamIds.includes(m.dreamId)));
        ...
        if (completedMeta) {
            ...
            latestCompleted = {
                ...
                summaryText: buildCompletionSummary(completedSession.output ?? null, maintenanceApplied),
        ...
        const phase = activeMeta
            ? resolveUiPhase(activeMeta, null, lockState, schedulerState)
