...
interface SettingsTemplatePanelOptions {
 ...
 * @returns 返回模板面板刷新函数。
 */
export function bindSettingsTemplatePanel(options: SettingsTemplatePanelOptions): { refreshTemplatePanelState: () => Promise<void> } {
    ...
    const templateTabEl = document.getElementById(options.ids.tabTemplateId);
    if (templateTabEl) {
        templateTabEl.addEventListener('click', (): void => {
            void refreshTemplatePanelState();
        });
    }

    const refreshBtn = document.getElementById(options.ids.templateRefreshBtnId);
    if (refreshBtn) {
    ...
    const forceRebuildBtn = document.getElementById(options.ids.templateForceRebuildBtnId);
    if (forceRebuildBtn) {
        forceRebuildBtn.addEventListener('click', async (): Promise<void> => {
            const memory = (window as any).STX?.memory;
    ...
    const templateSetActiveBtn = document.getElementById(options.ids.templateSetActiveBtnId);
    if (templateSetActiveBtn) {
        templateSetActiveBtn.addEventListener('click', async (): Promise<void> => {
            const memory = (window as any).STX?.memory;
            const activeSelectEl = document.getElementById(options.ids.templateActiveSelectId) as HTMLSelectElement | null;
            const lockEl = document.getElementById(options.ids.templateLockId) as HTMLInputElement | null;
