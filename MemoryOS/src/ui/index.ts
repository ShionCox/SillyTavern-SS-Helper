...
import { ensureSharedTooltip, applyTooltipCatalog } from '../../../SDK/sharedTooltip';
...
function applySettingsTooltips(): void {
    ...
    ensureSharedTooltip();
    const staleTips = Array.from(cardRoot.querySelectorAll<HTMLElement>('[data-tip]'));
    staleTips.forEach((node: HTMLElement) => node.removeAttribute('data-tip'));
...
function bindUiEvents() {
    const cardRoot = document.getElementById(IDS.cardId);
    if (cardRoot?.dataset.stxUiBound === '1') {
        applySettingsTooltips();
        return;
    }
    if (cardRoot) {
        cardRoot.dataset.stxUiBound = '1';
    }
