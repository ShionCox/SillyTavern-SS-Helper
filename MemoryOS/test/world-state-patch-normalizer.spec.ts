...
describe('world-state-patch-normalizer', (): void => {
    it('会把 mutation 的原始世界状态补丁规范成结构化记录', (): void => {
        ...
        expect(normalized.tags).toEqual(expect.arrayContaining(['nation', 'status', 'mutation_patch']));
    });
    ...
    it('会根据路径和文本识别角色目标与关系', (): void => {
        ...
        expect(goal.stateType).toBe('task');
    ...
    it('规范化后的 mutation 状态再次经过 state-manager 读取时不会漂移分类', (): void => {
        const normalized = normalizeWorldStatePatchValue(
        ...
        expect(node.scopeType).toBe('nation');
        expect(node.stateType).toBe('anomaly');
        expect(node.tags).toEqual(expect.arrayContaining(['mutation_patch']));
